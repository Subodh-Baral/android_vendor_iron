function __print_iron_functions_help() {
cat <<EOF
Additional ironOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- irongerrit:   A Git wrapper that fetches/pushes patch from/to ironOS Gerrit Review.
- ironrebase:   Rebase a Gerrit change and push it again.
- ironremote:   Add git remote for ironOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for ironOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the iron model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch iron_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/iron-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.iron.device | grep -q "$IRON_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $IRON_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function ironremote()
{
    ...
    local IRON_USER=$(git config --get review.review.iron-os.live.username)
    if [ -z "$IRON_USER" ]
    then
        git remote add iron ssh://review.iron-os.live:29418/$PFX$PROJECT
    else
        git remote add iron ssh://$IRON_USER@review.iron-os.live:29418/$PFX$PROJECT
    fi
    echo "Remote 'iron' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Iron-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.iron.device | grep -q "$IRON_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        ironremote
        git push iron HEAD:refs/heads/'$1'
    fi
    '
}
...
function irongerrit() {
    ...
    local user=`git config --get review.review.iron-os.live.username`
...
function ironrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ironOS Gerrit Rebase Usage: "
        echo "      ironrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.iron-os.live/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.iron.device | grep -q "$IRON_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/iron/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $IRON_FIXUP_COMMON_OUT ]; then
