...
func init() {
	android.RegisterModuleType("iron_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ironExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ironExpandVariables(ctx, dep_file)
	...
	cmd := ironExpandVariables(ctx, String(g.properties.Cmd))
